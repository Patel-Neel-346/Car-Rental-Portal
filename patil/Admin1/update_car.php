...
$id=$_GET['id'];
if($id){
    $statement=mysqli_query($conn,"select * from cars where id='$id'");
    ...
    if (move_uploaded_file($_FILES["car_image"]["tmp_name"], $target_file)) {
        $sql="Update cars SET car_name='$car_name',car_des='$car_type',car_image='$car_image',price_per_day='$price_per_day' where id='$id'";
...
<body class="bg-gray-100">
    ...
    <div class="flex">
        ...
        <div class="flex-1 ml-64 p-6">
            <h2 class="text-3xl font-bold text-gray-800 mb-8">Update Car</h2>
            ...
            <!-- Form to Add Car -->
            <form action="update_car.php?id=<?php echo $result['id'];?>" method="post" enctype="multipart/form-data" class="bg-white shadow-lg rounded-lg p-8">
                <div class="mb-6">
                    <label for="car_name" class="block text-gray-700 font-bold mb-2">Car Name</label>
                    <input type="text" name="car_name" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value="<?php echo  $result['car_name'] ?>" required>
                </div>

                <div class="mb-6">
                    <label for="car_type" class="block text-gray-700 font-bold mb-2">Car Description</label>
                    <input type="text" name="car_type" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value="<?php echo  $result['car_des'] ?>" required>
                </div>

               
                <div class="mb-6">
                    <label for="price_per_day" class="block text-gray-700 font-bold mb-2">Price Per Day</label>
                    <input type="number" name="price_per_day" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value="<?php echo  $result['price_per_day'] ?>" required>
                </div>

                <div class="mb-6">
                    <label for="car_image" class="block text-gray-700 font-bold mb-2">Car Image</label>
                    <input type="file" name="car_image" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" value="<?php echo  $result['car_image'] ?>" required>
                </div>

                <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600">Update Car</button>
            </form>
        </div>
    </div>
